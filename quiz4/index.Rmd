...
Plugging x = 8 in the equation, we get y = 75 + 6 * 8 = `r 75 + 6*8`. Hence sales equals \\$123,000.
...
We will use a t-statistic based on p = 0.01 and df = 15 - 2 = 13. From the table, we get `r qt(0.01, 13, lower.tail = F)`.
...
We know that $b_1 = r x s_y/s_x$ and $b_0 = \bar{y} - b_1 \times \bar{x}$. Plugging in values, we get $b_1 = sqrt(0.9231) \times 3.207/1.669$, which equals  `r b1`. Hence, $b_0 = 9.5 - `r b1` \times 5.75$, which equals  b0 = `r b0`. We can now write down the equation of the line as (C).
...
Plugging x = 6 in the equation, we get $\hat{y} = -1.115 + 1.846 \times 6$, which gives us `r -1.115 + 1.846*6`.
...
We can compute $\hat{y}$ for x = 8 as  $\hat{y} = -1.115 + 1.846 \times 8$, which gives us `r -1.115 + 1.846*8`. Hence, the residual is given by $y - \hat{y}$ which equals `r 15 - (-1.115 + 1.846*8)`
...
We know that $R^2 = 1 - SSE/SST$. Plugging in the given values, we get $R^2 = 1 - 80/500$, which gives us `r 1 - 80/500`
...
dfE = 15 - (5 + 1)
dfT = 15 - 1
adjrsq = 1 - ssesst*dfT/dfE
```


The adjusted Rsquare is given by 1 - (SSE/dfE)/(SST/dfT). We also know that $R^2 = 1 - SSE/SST$. So SSE/SST = 1 - $R^2$ = `r 1 - 0.995`. We also have dfE = `15 - (5 + 1)` and dfT = `15 - 1`. Plugging everything in, we get $1 - `r ssesst` \times `r dfT`/`r dfE`$, which gives us `r adjrsq`.
...
The R2 can be calculated from the ANOVA table as $R^2 = 3605/4820$, which gives us `r 3605/4820`.
...
We can plug in the values into the regression equation to get -1.6335 + 0.4485 x 4  + 4.2615 x 4  - 0.6517 x 13 which gives us `r -1.6335 + 0.4485 * 40  + 4.2615 * 4  - 0.6517 * 13`. Since y is measured in 100s of square feet, the answer is 2488 sqft which corresponds to (C).
...
Once again, we have 100 = -1.6335 + 0.4485 x I  + 4.2615 x 4  - 0.6517 x 16. Solving for I, we get I = (100 - (-1.6335  + 4.2615 * 4  - 0.6517 * 16))/0.4485, which gives us `r (100 - (-1.6335  + 4.2615 * 4  - 0.6517 * 16))/0.4485`. Hence, the answer is 211.85 (in thousands of dollars).
