...
which gives us a p-value of p = `r pval`. As the null hypothesis is not rejected, the significance level is smaller than `r pval`. The only value satisfying this condition is (C) 2%.
...
```{r}
library(visualizationTools)
po=power.t.test(n = 144, delta = -2, sd = 30, sig.level = 0.05,
  type = "one.sample", alternative="two.sided", strict = T)
plot(po)
```
