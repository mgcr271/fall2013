...
Let $X$ represent the amount of beer dispensed. For 1% of 12-ounce cups to overflow, the z-score corresponding to $x = 12$ should equal `r z`. Using $z = \frac{x - \mu}{\sigma}$, we can now solve for the mean, which gives us $\mu = `r x` - `r z` \times `r s`$. Hence, the answer is `r m`.
...
Let $X$ represent the amount of soft drink dispensed. For 2% of 6-ounce cups to overflow, the z-score corresponding to $x = 6$ should equal `r z`. Using $z = \frac{x - \mu}{\sigma}$, we can now solve for the mean, which gives us $\mu = `r x` - `r z` \times `r s`$. Hence, the answer is `r m`.
...
Let $X$ denote the mileage of the new brand. We wish to calculate $P(X > x)$, where $x = `r x`$. We can compute the z-score as $z = \frac{x - \mu}{\sigma}$, which gives us $z = `r z`$. From the standard normal table, we can find the area to the left of $z = `r z`$ to be `r p`. Since, we are interested in the area to the right, the answer is `r 1 - p`.
...
The z-score corresponding to the bottom 5% of drivers can be determined from the standard normal table as $z = `r z`$. Unstandardizing $z$, we can compute the corresponding mileage to be $x = \mu + z \times \sigma$, which gives `r x2`.
...
The z-score corresponding to the bottom `r p14`% of weights can be determined from the standard normal table as $z = `r z14`$ .  Unstandardizing $z$ , we can compute the corresponding weight to be $x = \mu + z \times \sigma$ , which gives `r x14`.
...
The z-score corresponding to the amount payed by the top `r p14b`% of families can be determined from the standard normal table as $z = `r z14b`$ .  Unstandardizing $z$ , we can compute the corresponding cost to be $x = \mu + z \times \sigma$ , which gives `r x14b`.
...
```{r faithful, fig.height = 5, fig.width = 10, fig.align = 'center'}
